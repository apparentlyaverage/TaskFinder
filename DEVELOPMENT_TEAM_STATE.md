...
> Last updated: 2026-06-28 — *Split transactional email into two category senders: SUPPORT (verify-email + password reset, reply-to a real inbox) and UPDATES (notifications + digest). Backend 222 tests; verified the support sender on a live stubbed reset. Needs Resend domain verify + env vars to send.*
...
- **CURRENT SPRINT GOAL:** Two-sender transactional email — a **support** address (password reset + new-account verification) and an **updates** address (notifications, digests).
- **STATUS:** ✅ Code done. `email.js` now has `EMAIL_FROM_SUPPORT` (+ `SUPPORT_REPLY_TO`) and `EMAIL_FROM_UPDATES`; `sendEmail` takes `from`/`replyTo`. Auth verify + reset → SUPPORT (reply-to a real inbox); `notify.js` + digest → UPDATES. Backend **222 tests**; verified locally that a password-reset stub logs `from: ReLivR Support <support@relivr.co.za>` + reply-to. **To actually send:** verify the `relivr.co.za` domain in Resend + set `RESEND_API_KEY` and the `EMAIL_FROM_*` env vars in Railway. (Prior: §7.11 scaling roadmap complete; biz01 showcase seeded.)
...
- **NEXT ACTION:** Email two-sender split shipped (code). **Owner to-do in Resend/DNS:** verify `relivr.co.za`, create `support@` (a real, monitored inbox for replies) + `updates@`, set `RESEND_API_KEY` + `EMAIL_FROM_SUPPORT`/`SUPPORT_REPLY_TO`/`EMAIL_FROM_UPDATES` in Railway. Optional follow-up: a real "new login detected" security alert (currently "new logins" = the new-account verification email). Then: UI redesign pass 2 / MVP-3 payments. **Re-seed demo:** `npm run seed:showcase`.
