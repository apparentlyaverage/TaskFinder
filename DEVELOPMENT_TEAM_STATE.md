...
> Last updated: 2026-06-30 — **Phase 2 nearly COMPLETE — B-group (B1–B7), A2 student-QR, C1–C3 handshakes, and D1–D3 scheduling all shipped; only A1 (ID verification) remains.** **D1/D2/D3** (migration 44): one **availability + bookings** system with a polymorphic host (user *or* business). A host publishes time slots; a guest books one (capacity-aware, conflict-safe). `server/routes/scheduling.js` — publish/list/delete slots, book/cancel, my-slots, my-bookings; **D3 reminders** via `jobs.sendBookingReminders` (notifies both parties ~1h before, `reminded_at`-guarded). Frontend: a member **Schedule page** (`/schedule` + nav) with `AvailabilityManager` + my-bookings; a **`BookingPanel`** on user *and* business public profiles; a **Bookings tab** on the business dashboard. Also fixed a dev-only gap: added `/scheduling` to the Vite proxy allowlist (prod/Vercel unaffected). Backend **267 tests green** (+13: scheduling 11, reminders 2); frontend builds clean. ⚠️ **Live UI not re-verified this session** — preview MCP kept dropping + the /auth/login rate-limit blocked the E2E; logic is fully test-covered + build-clean. Migration 44 on Neon. **Earlier in Phase 2:** C1–C3 price handshakes (migration 43, verified live), A2 student-discount QR (migration 42, verified live), B1–B7 task cards/filter/duration/deadline/extend/cancel. **Remaining: A1** — scaffold only, needs your **provider choice + SA legal sign-off**.
