...
> Last updated: 2026-06-30 — **Phase 2 "task core" (B-group) COMPLETE; Phase 1 done earlier.** This session shipped the whole B-group: **B1** image-free task cards + **B2** category colour-coding (left accent bar + colour-tinted icon/label chip, status & budget moved into the body; `CardCover` removed); **B7** bad-language filter — vendored **better-profane-words** (GPLv3, 2725 terms) as `server/profanity.js`, blocking intensity≥4 / slur / hate content on task create, bids, messages & reviews; **B3** expected-duration + **B4** bidding-deadline (migration 40; bid endpoint rejects late bids; detail shows a closed notice) + **B5** extend-deadline (`PATCH /tasks/:id/extend`, +7d capped 90d); **B6** cancellation reason (migration 41; captured + shown to declined bidders, profanity-checked). Backend **245 tests green**; frontend builds clean. ⚠️ **Preview MCP was disconnected most of this session** — B3–B7 verified by build + tests, **not visually**; eyeball the post-task form, task cards, and a cancel/extend on prod. Phase-2 remaining: **A2** student-discount QR, then **A1** ID verification (scaffold only — needs your provider choice + SA legal sign-off).
