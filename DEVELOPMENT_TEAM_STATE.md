...
> Last updated: 2026-06-28 — *Email now sends via Gmail SMTP (Nodemailer + App Password) — no domain needed. Two senders (Support / Updates) verified with a LIVE send to support.relivr@gmail.com (delivered:true). Resend kept as the future domain option. Backend 222 tests.*
...
- **CURRENT SPRINT GOAL:** Connect a real email service for transactional mail (no company domain yet) — owner chose `support.relivr@gmail.com`.
- **STATUS:** ✅ Done. `email.js` chooses a provider at runtime: **Gmail SMTP** (Nodemailer, if `GMAIL_USER`/`GMAIL_APP_PASSWORD` set) → **Resend** (if key) → stub. Gmail forces the From address to the account but keeps the category display name ("ReLivR Support"/"…Updates"). **Verified with a LIVE send** to `support.relivr@gmail.com` — both Support + Updates senders `delivered:true`. Backend **222 tests**. **Owner to-do:** set `GMAIL_USER` + `GMAIL_APP_PASSWORD` in Railway (that's all that's needed; display names default correctly). (Prior: §7.11 scaling roadmap complete; biz01 showcase seeded.)
...
- **NEXT ACTION:** Gmail SMTP wired + live-verified. **Owner to-do:** set `GMAIL_USER=support.relivr@gmail.com` + `GMAIL_APP_PASSWORD` in Railway → live email on prod. **Security:** the App Password was shared in chat during setup — rotate it (revoke + regenerate at myaccount.google.com/apppasswords) once set in Railway. **Migrate later:** when a company domain is registered, switch to Resend (set `RESEND_API_KEY` + verify the domain; drop GMAIL_*) — code already supports it. Then: UI redesign pass 2 / MVP-3 payments.
