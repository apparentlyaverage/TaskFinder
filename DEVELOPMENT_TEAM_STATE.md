...
> Last updated: 2026-06-30 — ***Phase 1 of the roadmap ([docs/FEATURE_BRIEF.md](docs/FEATURE_BRIEF.md)) is COMPLETE.** Shipped: H3 messaging icon (speech-bubble `ChatIcon`), A3 signup walkthrough, **A4 first-use coachmarks** (`FirstUseNote` on Messages + Local) + a "How ReLivR works" walkthrough-replay in the account menu, **F1a save/favourite providers** (migration 38: `follows.favourite`; star toggle on user/business profiles → favourites surface first with a ★ on the Following page), and **H2 feature-flag expansion** (migration 39: `rollout_roles` + `rollout_percent`; `/flags` now resolves per-viewer — role targeting + deterministic % bucket; admin Flags UI gains role pills + a rollout-% input; change history via the existing audit log). Backend **234 tests green**, frontend 3/3, builds clean. Verified live (student01): favourite toggle ☆→★ + Following ★, first-use note, walkthrough replay, and per-user `/flags` eval. *Not yet visually verified: the admin Flags targeting UI (no admin login this session — route is test-covered). Deferred per brief: flag per-campus + scheduled targeting; DB-backed onboarding; retainers (F1b, needs payments).*
