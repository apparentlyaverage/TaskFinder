...
> Last updated: 2026-06-29 — *Local directory ("/local") redesigned as an **Instagram-style feed**: single centered 500px column of business "posts" — avatar+name+category·address header, full-width square swipeable image carousel, follow + contact action row, clean caption (dedups a leading business-name echo). Carousel arrows stopPropagation so they no longer leak a tap through to the detail view (latent bug fixed). Preview MCP reconnected — **visually verified live** (mobile + desktop centering, carousel advance, tap-to-open, detail intact, no console errors) as student01@relivr.test against the seeded Neon directory. Earlier same-day: safe consistency polish (EmptyState, biz tabs, follower pill, wider messages list).*
