...
> Last updated: 2026-06-24 — *Follow/social graph BUILT & verified E2E (migration 35): follow users + businesses, follower counts, following feed, FollowButton + Following page. Backend 217 tests. §7.11.1 follow done.*
...
- **CURRENT SPRINT GOAL:** §7.11.1 — **follow / social graph** (users follow users + businesses).
- **STATUS:** ✅ Built + verified **E2E against the real DB**: `follows` table (migration 35); follow/unfollow, follower counts + viewer state, the "following" feed. `FollowButton` wired into public profiles + business pages; a **Following** page + nav. Real-DB probe: follow→201, state→{following:true, followers:1}, /follows/me→["Bean There Coffee"], unfollow→200. Backend **217 tests**; frontend builds clean. **Pending commit + push.** (Prior P1: god-mode + task TTL; Client History; Campus Deals; uploads.)
...
- **PENDING (backlog top):** 🆕 **Scaling roadmap (§7.11)** — ✅ P1 god-mode + task TTL/archive; ✅ P3 Client History; ✅ P2 **follow/social graph** — all delivered. **Remaining P2:** recurring specials (deals). Then: MVP-3 payments (TD-3, parked on company reg.); UI redesign **pass 2**; Campus Deals follow-ups; launch-day TODOs.
...
- **NEXT ACTION:** §7.11 P1 (god-mode + task TTL), P3 (Client History), and P2 (follow graph) all done. **Only remaining §7.11 item: P2 recurring specials** (extend `campus_deals` + `jobs.refreshRecurringDeals`). Awaiting owner steer. Campus Deals `/deals` opens publicly on 7 July with the gate.
...
| 3 | **Recurring specials** (deals) | **P2** | Extends the just-shipped Campus Deals; mirrors `task_templates`/`sendRecurring`. ← **next** |
