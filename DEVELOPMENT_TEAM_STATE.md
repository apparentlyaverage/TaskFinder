...
> Last updated: 2026-06-28 — *Rewrote the in-app legal pages (Terms, Privacy, Cookies, POPIA) to match the revised policy doc + reality: Paystack (not Stripe), 18+ (not Rhodes-gated), Information Officer uthando.mkwanazi@gmail.com, POPIA breach wording, real data-flow disclosures. Verified in-browser. (Prior: full email catalog.)*
...
- **CURRENT SPRINT GOAL:** Implement the revised legal/policy document into the live site.
- **STATUS:** ✅ Done. Rewrote the four in-app legal pages in `App.jsx` — `TermsPage` (doc §1–2), `PrivacyPage` (§3), `CookiesPage` (§4), `POPIAPage` (§5) — to match the revised doc **and** reality: Paystack (not Stripe), **18+ not Rhodes-gated**, removed the false "(PTY) Ltd registered" claim, Information Officer **uthando.mkwanazi@gmail.com** + general `support.relivr@gmail.com`, POPIA breach wording ("as soon as reasonably possible", eServices portal — not GDPR's 72h), and the real data-flow disclosures (deal-redemption shares your name with the business; device fingerprint; public profiles/reviews; named operators). Added Privacy subsections (lawful basis, cross-border, automated processing, children/special, direct marketing, portability). Frontend builds clean; **verified in-browser** (/privacy + /terms render with the new content, no Stripe, IO email present). **Owner to-do:** lawyer review; set `GMAIL_*` in Railway (prior email work).
...
- **NEXT ACTION:** Legal pages live-updated. **Owner to-do:** have a SA privacy attorney review (esp. retention periods + operator agreements); the source-of-truth doc is `RELIVR POLICY DOCUMENT (revised).docx` in Downloads. **Still inconsistent elsewhere:** TrustSafety/Help pages + landing still mention Stripe/Rhodes-SSO — a copy pass is a follow-up. Then: cookie-consent banner (to make §4 true), UI redesign pass 2, or MVP-3 payments.
