...
> Last updated: 2026-07-01 — **Phase 2 H1 (installable PWA + Web Push) shipped — Phase 2 is now COMPLETE except A1.** The app already had a manifest + 192/512 icons; added a **service worker** (`frontend/public/sw.js` — offline app-shell cache + `push`/`notificationclick` handlers) registered **production-only**, and a full **Web Push** path: migration 49 `push_subscriptions`, `server/push.js` (VAPID-gated `sendPush`, prunes dead 404/410 subs) + `/push` routes (`public-key`/`subscribe`/`unsubscribe`), wired into `createNotification` so every in-app notification also fires a best-effort push. A **Profile → Security "Push notifications"** Enable/Turn-off control runs the permission → subscribe → save handshake (shows **N/A** in dev/unsupported, **Blocked** if denied). Backend **296 tests green** (+7 push); frontend builds clean; migration 49 on Neon. ⚠️ **Set `VAPID_PUBLIC`/`VAPID_PRIVATE`/`VAPID_SUBJECT` in Railway** — the private key was shown in chat (rotate if concerned); push is prod-only so it's testable on the built/installed app, not the dev preview. **Live E2E still deferred** (auth rate-limit) — a consolidated visual pass on D1–D3, F1b/F2, E1–E5, H1 is owed. **Only A1 remains** — scaffold, needs your **provider choice + SA legal sign-off**.
>
