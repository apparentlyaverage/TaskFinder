...
> Last updated: 2026-07-01 — **Phase 2 H2 (feature-flag rollout system) COMPLETED — Phase 2 is now COMPLETE except A1.** H2's core (per-role, %-rollout, master kill-switch, audit) shipped in Phase 1; now added the brief's last two targeting axes: **per-campus targeting** (`rollout_campuses`, migration 50) and **scheduled enable/disable windows** (`enable_at`/`disable_at`). Both resolve **query-time** in `GET /flags` — order is kill-switch → schedule (vs NOW, no job) → role → campus → %; the viewer's campus is derived from `user_profiles.location_id` (looked up only when a flag targets campuses). Admin `PATCH /admin/flags/:key` accepts the new fields (validated, `null` clears, audited); the **Admin Flags UI** gains campus chips (empty = all campuses) + Enable-at/Disable-at datetime pickers. Backend **304 tests green** (+8); frontend builds clean; migration 50 on Neon. Also hardened the H1 service worker (best-effort shell caching so a renamed asset can't brick install). ⚠️ **Live E2E still deferred** (auth rate-limit + no admin login this session — the targeting UI is test-covered, not eyeballed); VAPID keys still to set in Railway. **Only A1 remains** — scaffold, needs your **provider choice + SA legal sign-off**.
>
> _Prior:_ **H1 (installable PWA + Web Push)** — service worker (offline shell + `push`/`notificationclick`, prod-only), migration 49 `push_subscriptions`, `server/push.js` (VAPID-gated `sendPush`) + `/push` routes wired into `createNotification`, and a Profile → Security push Enable/Turn-off control. **Set `VAPID_PUBLIC`/`VAPID_PRIVATE`/`VAPID_SUBJECT` in Railway** (private key was shown in chat — rotate if concerned).
>
> _Prior:_ shipped the whole **E-group (business B2B)**. **E1 reviews** (migration 48, `business_reviews`): anyone can rate/review a business (one per person, upsert, profanity-checked, owner can't self-review); rating shows on the profile + a reviews section with a leave-a-review form. **E3 public_code** + **E2 QR** (migration 47): every business has a short shareable code; the dashboard shows a **QR** (opens `/local?b=<id>`, which now deep-links straight to that profile). **E4 boost** (migration 47 `boosted_until`): a one-tap "Boost" (free during beta; billing at G1) surfaces a business first in Local with a **"Promoted"** badge. **E5 per-business toggles** (migration 47 `disabled_features[]`): admins switch deals/bookings/reviews/boost off per business (enforced server-side). Backend **289 tests green** (+10); frontend builds clean; migrations 47+48 on Neon. ⚠️ **Live E2E still deferred** (the /auth/login rate-limit) — everything is test-covered + build-clean; a visual pass on D1–D3, F1b/F2, and E1–E5 is owed once the limit clears. **Only A1 remains** — scaffold only, needs your **provider choice + SA legal sign-off**.
...
- **2026-07-01 — Phase 2 H2 completed: feature-flag rollout system (per-campus + scheduled).** H2's first half shipped in Phase 1 (migration 39: per-role `rollout_roles`, `%`-rollout with deterministic per-user bucketing, master `enabled` kill-switch, change audit via `writeAudit`). This finishes the brief's remaining two targeting axes. **Migration 50** adds `rollout_campuses UUID[]` (empty/NULL = all campuses), `enable_at`, `disable_at` to `feature_flags`. **Resolver (`flags.js`)** now evaluates in order: master kill-switch → scheduled window (`enable_at`/`disable_at` vs `NOW` — **query-time authoritative, no scheduler job**) → role → campus → %. The viewer's campus is derived from `user_profiles.location_id` walked up to its campus (`locations` is a campus→zone chain); that lookup only runs when some flag actually targets campuses, so untargeted /flags calls pay nothing extra. Anonymous / campus-less viewers are excluded from campus- or %-targeted flags (same rule as before). **Admin (`admin.js`)**: `PATCH /admin/flags/:key` now accepts `rollout_campuses`/`enable_at`/`disable_at` (validated `UUID[]`/`ISO8601`; `null` clears a target/schedule), returns + audits them; `GET /admin/flags` exposes the new columns. **Frontend `AdminFlags`**: a **Campuses** chip row (loads `GET /locations?kind=campus`; shows "all campuses" when none picked) + **Enable at / Disable at** `datetime-local` pickers with clear buttons, alongside the existing roles/rollout controls; stored UTC ↔ local input conversion via a `toLocalInput` helper. **Tests:** admin.test.js +8 (campus on/off resolution, schedule before/within/after, PATCH campuses+schedule 200, non-UUID campus 422, bad `enable_at` 422) → **backend 304 green**; frontend builds clean; migration 50 on Neon. **Verification:** unit + build only — no admin login this session and the `/auth/login` rate-limit is still tripped, so the targeting UI wasn't eyeballed (route + resolver are test-covered). **Kill-switch** requirement is met by the existing instant `enabled` toggle (audited). **Phase 2 now complete bar A1.** Also folded in an **H1 follow-up**: the service worker caches its shell **best-effort** (per-item, not atomic `addAll`) so a single renamed/missing asset can't silently break SW install + offline. Pushed to `main`.

- **2026-07-01 — Phase 2 H1: installable PWA + Web Push.** The PWA basics existed (manifest, 192/512 icons, `standalone`) — renamed the app to "ReLivR — Campus Marketplace" and added the missing pieces. **Service worker** (`frontend/public/sw.js`): caches the app shell, network-first navigations with an offline fallback, plus a `push` handler (renders `showNotification` from the JSON payload `{title,body,url,tag}`) and `notificationclick` (focus-or-open); registered in `main.jsx` **production-only** so dev keeps HMR clean. **Web Push:** migration 49 `push_subscriptions` (one row per device, unique endpoint, cascade on user delete); `server/push.js` — VAPID-gated (no-op unless `VAPID_PUBLIC`+`VAPID_PRIVATE` set, like the other optional providers), `sendPush(userId,payload)` fans out to every device and prunes dead 404/410 subs, plus `saveSubscription`/`removeSubscription`; `server/routes/push.js` — `GET /push/public-key`, `POST /push/subscribe`, `POST /push/unsubscribe` (auth + validated), mounted at `/push` (+ added to the Vite proxy allowlist). Wired `sendPush` into `notify.createNotification` (fire-and-forget, `tag=type`) so **every** in-app notification now also pushes. **Frontend:** a "Push notifications" row in **Profile → Security** with an Enable/Turn-off button that runs permission → `/push/public-key` → `pushManager.subscribe` → `POST /push/subscribe` (and the reverse to disable); on-mount state detection shows **N/A** where the SW isn't available (dev/unsupported browsers) and **Blocked** when permission is denied. **Tests:** new `push.test.js` (+7: public-key, subscribe 201/401/422, unsubscribe 200/422) → **backend 296 green**; frontend builds clean; migration 49 on Neon. **Owner to-do:** set `VAPID_PUBLIC`/`VAPID_PRIVATE`/`VAPID_SUBJECT` in Railway (keypair generated this session; **the private key was shown in chat → rotate if concerned**). Push is prod-only, so verify on the deployed/installed app, not the dev preview. Live E2E still deferred (auth rate-limit). **Phase 2 now complete bar A1.** Pushed to `main`.

- **2026-07-01 — Phase 2 E-group (E1–E5, business B2B):** **E1 reviews** — migration 48 `business_reviews` (one per user per business, upsert on re-submit); `POST/GET /businesses/:id/reviews` (profanity-checked, owner-self-review blocked, disabled-check), aggregate (avg + count) exposed on the list/detail/mine reads; frontend `BusinessReviews` (stars + form + list) on the IG profile + a rating stat in the header. **E3 public_code** — migration 47 adds a short unique code (backfilled from the id; generated on create); exposed on reads; `GET /businesses/code/:code` resolver. **E2 QR** — a `BizShareBoost` card on the dashboard renders a QR (via the existing `qrcode` dep) encoding `/local?b=<id>`; `LocalBrowse` now reads `?b=` on mount and deep-links straight to that profile. **E4 boost** — migration 47 `boosted_until`; `POST /businesses/mine/boost` (+7 days, free in beta, billing at G1); the public list orders boosted-first and cards show a "Promoted" badge. **E5 per-business toggles** — migration 47 `disabled_features TEXT[]`; `PATCH /businesses/:id/features` (admin); enforced in deal-create + review + boost; admin UI adds toggle chips (deals/bookings/reviews/boost) per business row. **Tests:** businesses.test.js +10 (reviews 4, boost 2, code+features 4) → **backend 289 green**; frontend builds clean. Migrations 47+48 on Neon. Live E2E deferred (auth rate-limit) — test-covered + build-clean. **Phase 2 now complete bar A1.** Pushed to `main`.

- **2026-07-01 — Phase 2 F1b (retainers) + F2 (deal notifications):** **F2** (migration 45 adds `campus_deals.expiring_notified_at`): `deals.js` now fans a new active deal out to the business's followers via `createNotification` (in-app + email per `email_frequency`, fire-and-forget so it never blocks create); `jobs.notifyExpiringDeals` warns followers of deals within 24h of expiry, guarded so each deal alerts once, wired into the 5-min tick. **F1b** (migration 46 adds `retainers`): a client sets up a recurring engagement with a provider — `POST /retainers` (validated; can't retain yourself; first task spawned immediately), `GET /retainers/mine` + `/incoming`, `POST /:id/cancel` (either party). `jobs.spawnRetainerTask` (shared by the route + runner) creates a task pre-assigned to the provider with `agreed_amount` = the rate; `jobs.runRetainers` spawns due cycles and advances `next_run_at` (weekly/monthly), wired into the hourly tick. Frontend: `RetainerSetup` (expandable form) on user profiles + `MyRetainers` (list/cancel) on the Following page; `/retainers` added to the Vite proxy. **F1a favourites already shipped in Phase 1.** Payment automation is deliberately deferred to G1 (escrow) — the retainer creates the work + notifies; the pair settle directly for now (same as every task today). **Tests:** retainers.test.js (8) + jobs (+4: notifyExpiringDeals, runRetainers) → **backend 279 green**; frontend builds clean. Migrations 45 + 46 on Neon. **Live E2E deferred** — the /auth/login 5-per-15-min rate-limit was tripped from earlier verification; owner will eyeball on prod once it clears. **Only A1 left in Phase 2.** Pushed to `main`.

- **2026-06-30 — Phase 2 D1–D3: scheduling & calendars (availability + bookings + reminders):** Migration 44 adds `availability_slots` (polymorphic host: `host_type` user|business + `host_id`, start/end, capacity, note) and `bookings` (slot, guest, status, `reminded_at`; unique active booking per guest+slot). New `server/routes/scheduling.js`: **publish** a slot (`POST /scheduling/slots` — user hosts as themselves, business hosts verify ownership), **list** a host's open future slots (capacity-aware `remaining`), **delete** a slot (cancels + notifies booked guests), **book** (`POST /scheduling/bookings` — future/capacity/not-own checks, 409 on full or double-book), **cancel** (guest or host), plus `/slots/mine` + `/bookings/mine`. **D3:** `jobs.sendBookingReminders` notifies both parties for bookings starting within the hour (`reminded_at`-guarded), wired into the 5-min scheduler tick. **Frontend:** shared `fmtSlot` + **`BookingPanel`** (book a host's slots — mounted on both the public **user profile** and **business profile**, hides when empty); **`AvailabilityManager`** (add/list/remove slots, shows who's booked); a member **`SchedulePage`** (`/schedule`, added to NAV + top-bar links) = my availability + my bookings; a **Bookings tab** on the business dashboard (`AvailabilityManager hostType="business"`). Mounted at `/scheduling` in app.js. **Dev fix:** `/scheduling` was missing from the Vite proxy allowlist (so dev browser calls fell through to the SPA) — added it; prod/Vercel calls the backend directly so was unaffected. **Tests:** scheduling +11 (publish/list/book/cancel/auth/capacity), jobs +2 (reminder) → **backend 267 green**; frontend builds clean. Migration 44 on Neon. ⚠️ **Live UI verification was blocked this session** — the preview MCP repeatedly disconnected and the `/auth/login` 5-per-15-min rate-limit (hit during E2E retries) stopped a clean run; the API E2E + UI screenshot are the one thing to eyeball on prod. Pushed to `main`.
