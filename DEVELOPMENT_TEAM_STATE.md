...
> Last updated: 2026-06-28 — *Full email catalog (`emails.js` + `docs/EMAILS.md`): verify, reset, password-changed, new-sign-in alert (mig 37 known_logins), welcome, suspend/reinstate/delete, waitlist — branded HTML, correct sender per type. Wired across auth/profile/admin/beta. Backend 227 tests; new templates LIVE-sent via Gmail (delivered:true).*
...
- **CURRENT SPRINT GOAL:** A complete transactional-email catalog — plan every email type + implement them.
- **STATUS:** ✅ Done. Plan in `docs/EMAILS.md`; `emails.js` is the typed catalog (branded HTML+text, correct support/updates sender per type). **New + wired:** password-changed, **new-sign-in alert** (mig 37 `known_logins`, fingerprint = sha256 UA, skips first sign-in), welcome (on verify), account suspended/reinstated/deleted, waitlist confirmation; verify + reset refactored into the catalog. Activity mail (bids/messages/reviews/follows/…) already flows via `createNotification`. Provider still Gmail SMTP → Resend → stub. Backend **227 tests**; the new templates **LIVE-sent** to `support.relivr@gmail.com` (`delivered:true`). **Owner to-do:** set `GMAIL_USER` + `GMAIL_APP_PASSWORD` in Railway, then rotate the App Password.
...
- **NEXT ACTION:** Email catalog complete + live-verified. **Owner to-do:** set `GMAIL_USER`+`GMAIL_APP_PASSWORD` in Railway, then **rotate the App Password** (shared in chat during setup). **Future email work (in `docs/EMAILS.md`):** task-deadline reminder, weekly business summary, payment receipts (MVP-3), re-engagement, email-change confirm. Then: UI redesign pass 2 / MVP-3 payments.
