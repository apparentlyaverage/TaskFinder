...
> Last updated: 2026-07-01 — **Phase 2 all but done — B-group, A2, C1–C3, D1–D3, and now F1b + F2 shipped; only A1 (ID verification) remains.** **F2 deal notifications** (migration 45): when a business posts a deal, its followers are alerted (`notifyDealFollowers`, in-app + email per preference); a `jobs.notifyExpiringDeals` sweep warns followers when a deal is within 24h of expiring (`expiring_notified_at`-guarded). **F1b retainers** (migration 46): a client sets up a recurring engagement with a provider — `server/routes/retainers.js` (create/list/incoming/cancel); `jobs.runRetainers` spawns a task pre-assigned to the provider each cadence (agreed_amount = the rate, the C2 source of truth); the first task fires on setup. Frontend: **`RetainerSetup`** on a user's profile + a **`MyRetainers`** manager on the Following page. **F1a (favourites) shipped back in Phase 1.** *Auto-charge on retainers hooks in with payments (G1) — until then the pair settle directly, same as any task today.* Backend **279 tests green** (+12: retainers 8, deal/retainer jobs 4); frontend builds clean. Migrations 45+46 on Neon. ⚠️ **Live E2E deferred** (the /auth/login rate-limit) — logic is fully test-covered + build-clean; owner to eyeball on prod. **Remaining: A1** — scaffold only, needs your **provider choice + SA legal sign-off**.
