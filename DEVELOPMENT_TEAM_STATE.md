...
> Last updated: 2026-06-30 — *Local ("/local") reworked into a true **Instagram experience**: the directory is an **explore grid** of image-forward business tiles (cover + "▦ N" multi-photo badge + mini avatar/name), and tapping a business opens an **Instagram-style profile** (circular avatar, name, Follow, "N photos" stat, bio + contact actions, then a **3-column photo grid**). Tapping any photo opens a **full-screen swipeable lightbox** (arrows / keyboard / touch-swipe, counter + dots, body-scroll lock) — **portalled to `document.body`** so it escapes the page's transformed stacking context and covers the sticky header/nav. **Visually verified live** as student01@relivr.test against the seeded Neon directory (grid 3-col @tablet, profile, lightbox open/nav/escape, full-viewport coverage, no console errors). Replaced the short-lived feed-of-posts (`BizFeedPost` removed).*
