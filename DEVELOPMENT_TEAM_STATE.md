...
> Last updated: 2026-06-24 — *Recurring specials BUILT (migration 36 + jobs.refreshRecurringDeals) → §7.11 scaling roadmap COMPLETE. biz01@relivr.test seeded to showcase all new features (recurring deals, redemptions/Client History, followers). Backend 220 tests.*
...
- **CURRENT SPRINT GOAL:** §7.11.2 — **recurring specials** + showcase-seed `biz01` so all the new scaling features are viewable.
- **STATUS:** ✅ Recurring deals built (migration 36: `recurrence`/`recurrence_until`/`active_window_s`; `jobs.refreshRecurringDeals()` refresh-in-place on the scheduler; "Repeat" selector in the deal form + ↻ badge). `biz01@relivr.test` seeded (`npm run seed:showcase`): 3 deals (weekly/daily/one-off), 8 followers, 12 redemptions (8 unique, 4 repeat). Backend **220 tests**; frontend builds clean. **→ §7.11 scaling roadmap COMPLETE.** Pending commit + push + prod verify.
...
- **NEXT ACTION:** §7.11 scaling roadmap fully delivered + `biz01` seeded for demo. Awaiting owner steer — likely **UI redesign pass 2** (in-app surfaces) or **MVP-3 payments** (still parked on company registration). Campus Deals `/deals` opens publicly on 7 July with the gate. **To re-seed the demo:** `npm run seed:showcase`.
